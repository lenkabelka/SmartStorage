...
class ThingInformation(QWidget):
    def __init__(self, thing: Thing):
        ...
        #self.setMinimumSize(int(screen_width * width_coef), int(screen_height * height_coef))
        ...
        if thing.thing_images:
            ...
            container_images.resize(self.width(), int(0.4 * self.height()))
            ...
            for item in thing.thing_images:
                ...
                image_widget = image_container.ImageContainer(item, 0.3 * self.height())
                image_widget.setContextMenuPolicy(Qt.ContextMenuPolicy.NoContextMenu)
                #image_widget.setMinimumSize(int(0.3 * self.height()), int(self.width()))
                #image_widget.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
